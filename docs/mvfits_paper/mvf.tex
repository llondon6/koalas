...
\begin{abstract}
  %
  Models of black hole properties are an important component to the ongoing direct detection of gravitational waves from black hole binaries.
  %
  In particular, model based detection, and subsequent parameter estimation are directly enabled by the modeling of information related to perturbed Kerr black holes.
  %
  ...
  %
  A greedy-multivariate-polynomial (GMVP) regression method and greedy-multivariate-rational (GMVR) regression method are presented for the automated modeling of polynomial and rational functions.
  %
...
\end{align}
%
\par In the first and second lines of \eqn{hrd}, we relate the observable \gw{} polarizations, $h_+$ and $h_\times$, to the analytically understood morphology of the time domain waveform.
%
...
%
This latter point is key to the perspective of \gmvp{}: given training data thought to be related to a smooth multivariate function, it may be well approximated by a truncated series expansion in an appropriate coordinate basis.
%
...
%
At this stage, we will also explicitly consider the $j^\mathrm{th}$ samples of the domain, and so refer to $\vec{x}$ as $\vec{x}_j$.
%
%
\begin{figure*}[htb]
  %
...
%
Put another way, rather than two simultaneous greedy optimizations over $R$ and $V-1$ symbols (with $(V-1)R$ iterations), a single greedy process over $V+R$ symbols is performed.
%
...
%
When considering many noise realizations, we find that sample noise and residuals have an average correlation of $99.46\%$.
%
...
%
%
\par In this example case, \gmvr{} correctly recovers the functional form of the input data. But, in general, \gmvr{} and related techniques, having no knowledge of , will attempt to model minor correlations and offsets within the training data's noise.
%
...
%
Namely, we will impose a zero-damping constraint: some frequencies are real as $\jf \rightarrow 1$.
%
...
\end{align}
%
appropriately linearizes the sharp behavior of each frequency near $\jf=1$, and maps $-1 \le \jf \le 1$ onto $0 \le \kappa \le 1$.
%
...
%
For the right and left panels, simple polynomial behavior of each curve is a result of the displayed linear domain in $\kappa(\jf,\ell,m)$.
%
In the left panel, we have scaled $1/\tau\lmn$ by factors for $m/(n+1/2)$ to place the \qnm{s} with $n=0$ and $n=1$ at approximately the same scale.
%
...
\end{widetext}
%
\par \fig{fig:qnm}'s top left and right panels demonstrate the effect of mapping $\jf$ onto $\kappa$.
%
In particular, it is shown that the two solution branches ($\jf>0$ and $\jf<0$) naturally form a single family of solutions when accounting for the sign of the \bh{}'s oriented spin.
%
Concurrently, the use of $\kappa$ as a domain variable has the desirable effect of making each $\cw\lmn$ and $\tau\lmn$ approximately parabolic.
%
We note that, in the asymptotic vicinity of $\jf=1$, the \qnm{} frequencies and decay times are known to have solutions that are asymptoticly degenerate.
%
In allowing \eqns{eq:cw_fit_1}{eq:cw_fit_9} to extrapolate to $\jf \sim 1$, we do not explicitly account for this additional effect.
%
\par Lastly, \fig{fig:qnm}'s bottom two panels show residual errors.
%
...
%
\par While it is possible to enforce extremal Kerr and Schwarzschild limiting conditions for $\sigma_{\LMlmn}$, we find it effective to first use \gmvr{} to determine a functional form that works for all $\sigma_{\LMlmn}$, and then apply this form to the modeling of all $\sigma_{\LMlmn}$.
%
