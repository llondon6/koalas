...
\end{align}
%
From the first to second line of \eqn{eq:mvt}, we have used the definition of the exponential function (i.e. its series expansion form).
%
...
%
\par The combination of these two ideas alone results in an algorithm prone to a deficit of stepwise methods: there is the very likely possibility that the algorithm would confuse correlated basis vectors (e.g. $x^2$ may be confused for $x^4$).
%
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    %
    \State {\bf Input:} $ \{ \bf{x}, \bf{y}, $max\_degree = 6$, tol \}$
    \vskip 10pt
...
\begin{align}
  \label{eq:rat1}
  f(\vec{x}) = \muf + \sif \; \frac{ \sum_{r=0}^{R} a_r \, \phi_{r}(\vec{x}) }{ 1 - \sum_{v=0}^{V} b_v \, \phi_{v}(\vec{x}) } \;,
\end{align}
%
where $\muf$ is the additive mean of $f(\vec{x})$, and $\sif$ is the standard deviation of $f(\vec{x})$ from its mean.
%
\par While it is tempting to embrace \eqn{eq:rat1}'s $f({\vec{x}})$ as a nonlinear function and so resort to nonlinear modeling methods, note that a reformulation reveals an underlying linear structure.
%
Namely, if we let
%
\begin{align}
...
\begin{align}
  \label{eq:rat2}
  g = \sum_{r=0}^{R} a_r \, \phi_{r}(\vec{x}) + g\, \sum_{v=0}^{V} b_v \, \phi_{v}(\vec{x}) \; .
\end{align}
%
...
\begin{align}
  ...
  z_k = \left\{ \begin{array}{cc}
        ...
        b_k, & \quad \;\; \;\text{for }R+1\leq k\leq R+V+1
      \end{array} \right\}
\end{align}
%
and
...
\begin{align}
  ...
  \psi_k(\vec{x}_j) = \left\{\begin{array}{cc}
        ...
        \phi_k(\vec{x}_j) g , & \quad \;\; \;\text{for }R+1\leq k\leq R+V+1
        \end{array} \right\}
\end{align}
%
...
%
\par But note that $\hat{P}$'s depending on $g$ signals that there is more to be done.
%
In particular, let us briefly consider the effect of noise on $g$, e.g. $g \rightarrow g+n$.
%
...
%
\par The key to robustly solving for $\vec{\alpha}$ lies in iterative refinement.
%
Specifically, we note that \eqn{eq:rat2} may be modified iteratively minimize the impact of numerical noise on the eventual solution, $\vec{\alpha}$.
%
To this end, if we define $g^{(0)}$ to be \eqn{eq:rat2} evaluated with $\vec{\alpha}$ as given by \eqn{eq:pinv2}, and note that by construction, $g^{(0)}$ is less effected by (zero mean) noise in $g$, then a further refinement may be obtained by
%
\begin{align}
  \label{eq:rat2}
  g^{(1)} = \sum_{r=0}^{R} a_r \, \phi_{r}(\vec{x}) + g^{(0)}\, \sum_{v=0}^{V} b_v \, \phi_{v}(\vec{x}) \; .
\end{align}
%
\eqn{eq:rat2} straightforwardly generalizes to
%
\begin{align}
  \label{eq:rat3}
  g^{(n+1)} = \sum_{r=0}^{R} a_r \, \phi_{r}(\vec{x}) + g^{(n)}\, \sum_{v=0}^{V} b_v \, \phi_{v}(\vec{x}) \; .
\end{align}
%
% GMVR
\hspace{1cm}
