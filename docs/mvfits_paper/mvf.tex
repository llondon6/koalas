...
\begin{algorithm}[H]
  %
  \caption{A positive (forward) greedy algorithm, \texttt{PGREEDY}. Note that a required input, $\mathcal{A}$, is a function that takes in a list of basis symbols, and outputs an estimator of fit error. In this setting, $\mathcal{A}$ is assumed to have access to peripheral information, such as the training data.}
  \label{alg:pgreedy}
  %
  \begin{algorithmic}[1]
    %
...
%
Each term is an element of the cartesian inner-product of the power-set of each model dimension. That is $\mlam_{bulk} = \{x_0,x_1 .. x_N, ... x_0^1, x_0^2, x_0^3, ...  , (x_0^K x_1^K ... x_N^K) \}$.
%
...
%
Recall that a term such as $x_0x_1x_3^2$ has an effective order of 4.
%
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    %
    \State {\bf Input:} $ \{ \bf{x}, \bf{y}, $max\_order = 6$ \}$
    \vskip 10pt
    \State Define bulk symbol space using cartesian inner-product of the power set of each basis symbol
