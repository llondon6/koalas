...
%
Worst, in cases where the underlying dataset is best described by a rational function, a rational model with maximum monomial degree $K$ will typically outperform a polynomial model of maximum degree $K$ with the same number of coefficients as the rational model.
%
...
\begin{align}
  \label{eq:ratvar}
  \operatorname{Var}\left(\vec{d}\right)  = \frac{1}{J}\sum_{j=0}^{J}\left(d_j - \frac{1}{J}\sum_{u=0}^{J}d_u\right)^2
\end{align}
%
% \hspace{1cm}
{\scriptsize
...
%
here, the sharp feature near $\jf=0$ results from their modeling counter and co-rotating \qnm{} as two different curves. A residual error is also shown as an orange dashed line for the $\cw_{220}$ model presented in Ref.~\cite{Nagar:2018zoe}. This is a rational model with 14 fitted parameters compared to the polynomial model with 10 fitted parameters defined in \eqn{eq:cw_fit_1}. We observe that Ref.~\cite{Nagar:2018zoe} performs better overall while performing worse for higher $\jf$.
%
